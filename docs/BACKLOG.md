...
- Replay of captured fixtures (`rcc send --from test/fixtures/events/real/….json`) — would let a handler be tested against *real* payloads, not only generated ones; needs redaction rules (see above). Motivated by `docs/research/market-validation-2026-08-29.md` §7 (fidelity is the main objection to a synthetic simulator).
- `PRODUCT_CHANGE` / `TRANSFER` generation — both are in the official flows and both appear in ordering-bug reports (HN 2021, forum 2025); need plan-change and cross-user semantics in the state machine.
- Exported TypeScript types for handler authors (`import type { WebhookEnvelope, Event } from "reveclicat"`) — the schemas already exist; `@puzzmo/revenue-cat-webhook-types` shows the demand. Cheap; do it when a user asks.
- RevenueCat opt-in HMAC header (`X-RevenueCat-Webhook-Signature`) as a `--sign <secret>` flag — only if a real capture of a signed delivery is available first (fidelity rule).
