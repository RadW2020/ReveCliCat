...
## 2026-08-29 · Epic 7 · Google Play in the generator (T-070…T-072)
- Sources first (`specs/F7-google-play.md`, `payload-sources.md`): RevenueCat S2 (`<subscription_id>:<base_plan_id>`, all 7 types for Play, `is_family_share` false outside App Store), S3 PRODUCT_CHANGE sample (`GPA.1234-1234-1234-12345`), and Google's `Purchase.getOrderId()` reference for the `..N` renewal suffix (fetched from developer.android.com — the RevenueCat docs are silent on it).
- Tests first (7 new + 3 adjusted): GPA ids, `..0/..1/..2` renewals (trial conversion = `..0`, recovery keeps counting), new order on resubscription, App Store unchanged, schema validity + seed determinism, `parseStore` message lists both stores, scenario `store: play_store` with per-store default `product_id` and explicit override; examples/init expect 7 scenarios.
...
## 2026-08-29 · Market validation and README repositioning
- Maintainer asked what value the tool adds, to whom, how people do it today, and for up-to-date online testimony. Three parallel research passes (developer pain, competitive landscape, audience/channels); every quote checked on the page. Written up in `docs/research/market-validation-2026-08-29.md`.
- The one finding that changes the product's story: **RevenueCat published an official CLI (`@revenuecat/cli`, `rc`) on 2026-08-25** — verified on the registry and in its README. It manages webhook configuration and simulates Test Store purchases; it does not trigger chosen events, chain lifecycles, listen locally or assert. So "the Stripe CLI that RevenueCat does not have" was no longer true; the defensible framing is "`stripe trigger` + `stripe listen` for RevenueCat — what `rc` does not do".
- Evidence in one line: RC staff confirm in writing (Sep 2024, Jul 2025) that there is no local/CLI path and recommend ngrok; ≥ 9 forum threads report sandbox never delivering RENEWAL/CANCELLATION/EXPIRATION; pick-an-event and resend were requested Dec 2025 and "shared internally". Counter-evidence recorded too: niche, episodic, no willingness-to-pay signal, two 0-star OSS attempts at the same idea.
- README: tagline now names App Store, Google Play and Stripe (the maintainer's correction — the tool is not Stripe-only); "Why it exists" gets a dated comparison table (dashboard test event / sandbox + Test Store / `rc` / ReveCliCat); "Fidelity & scope" leads with the real captures (TEST, promotional, 7-event Stripe) and the three defects they found; stale "six scenarios" → eight, `TEST`-still-provisional note removed (captured since T-004/T-063). `rcc init` description no longer hard-codes the count (snapshot updated).
- Icebox: fixture replay, `PRODUCT_CHANGE`/`TRANSFER`, exported TS types, HMAC flag — as candidates with their motivation, not scheduled.
- Promotion tasks (forum replies, Shipaton 2026 window, Reddit/HN/dev.to, `rc` collaboration) live in a local git-ignored `PROMO-TODO.md`: they are the maintainer's, not the product's.
- Gates: typecheck ✓ · lint ✓ · tests 245/245 ✓.
