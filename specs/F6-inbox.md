...
## `rcc tail` (T-062) — the client

`rcc tail --inbox https://hooks.example.com --token <t> [--since <seq>|--all] [--forward http://localhost:3000/webhook] [--verbose]`

- Connects to `/events/stream`, prints each event with the same one-line format as `rcc listen` (`time  TYPE  app_user_id  product_id`) plus `real` marker; `--verbose` prints the body.
